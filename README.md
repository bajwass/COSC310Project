...
Google Feed API
- Implemented a Google Feed API using the following tutorial:
- If something is not within the scope of the bot, the user has the ability to google search the answer to said input given.
- To run the google API the user has to do the following :

    Navigate to the project folder and enter the following 
        `rasa run --enable-api -p 5005`
    This is what allows rasa to run the implemented API

    Next is to run the custom actions implemeted into rasa already
        `rasa run actions`

    Then we deploy the Google Assistant Skill, using the Project ID
       `gactions update --action_package action.json --project jokebot-ddec8`

    Last we run the Google API to test
        `gactions test --action_package action.json --project jokebot-ddec8`

    Once this is run, a link will be provided in the terminal which you click on to run the Google API.


Wikipedia API
    - This implemented API allows the user to get a wikipedia explanation for certain parts of the chatbot response that might not make sense

    - Running the Wikipedia goes as follows:

    First you have to install wikipedia through the console by using
          `pip install wikipedia`

    Once that is done you follow the same rules as above to run the chatbot on the web browser you are using.
    ...
    **navigate to the `GUIWebsite` folder** using something like `cd GUIWebsite` from the root folder.
    ...
    Once that is done, type `localhost:8000` into the browser of your choice.
